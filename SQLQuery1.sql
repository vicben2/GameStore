...
AS
BEGIN
	SELECT G.GAME_TITLE, U.USERNAME, SUM(LINE_TOTAL) AS TOTAL_REVENUE, SUM(LINE_TOTAL * 0.3) AS ADMIN_SHARE, SUM(LINE_TOTAL * 0.7) AS DEV_SHARE FROM GAME G
	JOIN STORE_USER U ON G.DEV_ID = U.USER_ID
	JOIN ORDER_DETAIL OD ON G.GAME_ID = OD.GAME_ID
	JOIN ORDER_HEADER OH ON OD.ORDER_HEADER_ID = OH.ORDER_HEADER_ID
	GROUP BY G.GAME_TITLE, U.USERNAME
END
